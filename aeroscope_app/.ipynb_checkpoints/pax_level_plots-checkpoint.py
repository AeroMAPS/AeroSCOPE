...
def pax_map_plot(flights_gpb_df):
    ...
    for i in range(len(flights_gpb_df)):
        fig.add_trace(
            go.Scattergeo(
                ...
                mode='lines',
                line=dict(width=flights_gpb_df['CO2 Ppax'][i] / (1.5*meanwidth), color="#023047"),
                opacity=0.8
            )
        )
    ...
    fig.add_trace(go.Scattergeo(
        ...
        mode='markers',
        marker=dict(
            size=flights_gpb_df['CO2 Ppax'] / (.01 * flights_gpb_df['CO2 Ppax'].mean()),
            color='#ffb703',
            sizemode='area',
            opacity=0.8,
    ...
    fig.update_layout(showlegend=False, height=800, title='Route values for {}'.format('CO2 Ppax'))
